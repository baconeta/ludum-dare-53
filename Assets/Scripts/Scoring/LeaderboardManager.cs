...
{
    [Header("Text Fields")]
    [Tooltip("Highscore 'Name' Column")]
    [SerializeField]
    private TextMeshProUGUI nameColumn;

    [Tooltip("Highscore 'Score' Column")]
    [SerializeField]
    private TextMeshProUGUI scoreColumn;

    [Tooltip("Row count | How many highscores to show at once")]
    [SerializeField]
    private int maximumEntries = 12;

    private const string SubmitScoreUri =
        "https://ludum-dare-53-score-server.herokuapp.com/api/scores?user={0}&score={1}";
    ...
    void Start()
    {
        if (nameColumn != default)
        {
            ...
            Callback callback = new(UpdateTextFields);
    ...
    {
        using UnityWebRequest ping = UnityWebRequest.PostWwwForm(String.Format(SubmitScoreUri, user, score), "");
        yield return ping.SendWebRequest();
